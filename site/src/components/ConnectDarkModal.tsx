...
export default function ConnectDarkModal() {
 ...
 <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
    ...
    {allDefaultWallets?.map((wallet, i) => (
       <div key={i} className={`flex justify-between border ${themeConfig === "dark" && "border-gray-700/70 hover:border-gray-700/60"} border-gray-300/80 py-1.5 px-3 rounded-md cursor-pointer hover:animate-scaleSlow hover:border-gray-400 my-5`} >
       <h2 className="capitalize font-medium "> {wallet.name}</h2>
        <img  src={wallet?.logoUrls[0]} className="w-7 h-7 rounded-full"   />
       </div>
       ))}
       
       <div className={`flex gap-2 items-center justify-center ${themeConfig === "dark" ? "text-gray-400 hover:text-gray-300" : "text-gray-500 hover:text-gray-800"} cursor-pointer  text-sm`} > 
